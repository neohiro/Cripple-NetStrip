...
class NetStripEngine:
    ...
    def _blocklist_updater_loop(self):
        ...
        # Initial sleep for 5 seconds after boot before running first background update check
        for _ in range(5):
            if not self.is_running:
                return
            time.sleep(1)
        ...
        try:
            ...
            with self.db.lock:
                with self.db._get_connection() as conn:
                    rows = conn.execute("SELECT key, value FROM settings WHERE key LIKE 'backup_dns_%'").fetchall()
                    for row in rows:
                        key = row['key']
                        original_dns = row['value']
                        interface = key.replace('backup_dns_', '')
                        self.platform.restore_system_dns(interface, original_dns)
                        restored.add(interface)
                        
            # Also safely restore currently active ones just in case they weren't in the DB
            for interface in self.platform.get_active_interfaces():
                if interface not in restored:
                    self.platform.restore_system_dns(interface, "dhcp")
        except Exception as e:
    ...
    def set_killswitch(self, active: bool, broadcast_lan: bool = True):
        ...
        if active:
            ...
            # Unconditionally enable LAN shield as part of Ghost Mode lockdown
            ...
            # Restore Normal Mode settings accurately
            ...
            if self.db.get_setting("killswitch_harden_adapters", "true") == "true":
                try:
                    # Only disable adapter hardening if it's NOT enabled by the user in Normal mode
                    if self.db.get_setting("block_system_connections", "false") != "true":
                        if hasattr(self.platform, 'harden_network_adapters'):
                            self.platform.harden_network_adapters(enable_hardening=False)
                except Exception as e:
