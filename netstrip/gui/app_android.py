...
class NetStripApp(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        # --- Top App Bar ---
        self.top_bar = ctk.CTkFrame(self, fg_color=Colors.BG_PANEL, corner_radius=0, height=50)
        ...
        controls_frame.grid(row=0, column=2, padx=16, sticky="e")

        self.btn_sound_toggle = ctk.CTkButton(
            controls_frame, text="🔊", font=(Fonts.FAMILY_PRIMARY[0], 20), width=36, height=36,
        ...
        self.btn_killswitch.pack(side="left")

        try:
            from netstrip.gui.hovertip import FadingHovertip
            FadingHovertip(self.btn_killswitch, "Click to toggle Killswitch mode.", hover_delay=400)
            FadingHovertip(self.btn_sound_toggle, "Toggle sounds.", hover_delay=400)
        except Exception:
            pass
        ...
        self.bottom_nav = ctk.CTkFrame(self, fg_color=Colors.BG_PANEL, corner_radius=0, height=60, border_width=1, border_color=Colors.BORDER_SUBTLE)
    ...
    def _add_bottom_nav_btn(self, col, text, icon, view_class):
        btn = ctk.CTkButton(
            self.bottom_nav,
            text=f"{icon}\n{text}",
            corner_radius=8,
        ...
        btn.grid(row=0, column=col, sticky="nsew", padx=4, pady=4)
        ...
        try:
            if self.badge_label:
                if count > 0:
                    ...
                    self.badge_label.place(relx=0.7, rely=0.2, anchor="center")
                else:
